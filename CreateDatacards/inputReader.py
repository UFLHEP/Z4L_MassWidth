    ...
    def readInputs(self):
        ...
        for line in open(self.theInput,'r'):
            ...
            if f[0].lower().startswith("rate"):
                ...
                if f[1].lower().startswith("signal"):
                    self.signal_rate = float(f[2])
                    if len(f) == 4: self.signal_lumi = float(f[3])
...
#        #if not self.goodEntry(self.zjetsShape_p4): raise RuntimeError, "{0} is not set.  Check inputs!".format("zjetsShape_p4")
        ...
        if not self.goodEntry(self.signal_lumi): self.signal_lumi = self.lumi
      ...
      ## Set dictionary entries to be passed to datacard class ##
        ...
        dict['signal_rate'] = self.signal_rate
        ...
        dict['signal_lumi'] = float(self.signal_lumi)
