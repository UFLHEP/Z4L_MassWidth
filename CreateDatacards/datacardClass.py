...
class datacardClass(object):
    ...
    def setVariables(self, theMH,theOutputDir,theInputs,theTemplateDir,theMassError,theis2D,theUseMEKD):
        ...
        if self.bIncludingError: print "~~~~ Using M4L Errors ~~~~" 
        self.is2D = theis2D
    ...
    def makeMassShapesYields(self,theMH,theOutputDir,theInputs,theTemplateDir,theMassError,theis2D,theUseMEKD):
        ...
        ## -------------------------- SIGNAL SHAPE ----------------------------------- ##
        bins = 3000
        if(self.bUseCBnoConvolution): bins = 3000
        if(self.bIncludingError): bins = 3000

        self.CMS_zz4l_mass = ROOT.RooRealVar("CMS_zz4l_mass","CMS_zz4l_mass",self.inputs['low_M'],self.inputs['high_M'])
        self.CMS_zz4l_mass.setBins(bins) 
...
#         self.rhfXsBrFuncV_5 = ROOT.RooHistFunc(rhfname,rhfname, ROOT.RooArgSet(self.MH), self.rdhXsBrFuncV_5, 1)
        ...
        name = "MH"
        self.MH = ROOT.RooRealVar(name,"MH",91.1876)
        self.MH.setConstant(True)
        name = "HiggsDecayWidth"
        self.HiggsDecayWidth = ROOT.RooRealVar(name,"HiggsDecayWidth",2.4952)
        self.HiggsDecayWidth.setConstant(True)

        self.one = ROOT.RooRealVar("one","one",1.0)
        self.one.setConstant(True)
    
        #self.MH.setVal( mean_BW_d )
        ...
        name2 = "CMS_zz4l_alpha2_{0:.0f}_centralValue".format(self.channel)
        if self.isHighMass :
            ...
            self.rfv_alpha2_CB = ROOT.RooFormulaVar(name2,self.inputs['alpha2_CB_shape'], ROOT.RooArgList(self.MH))
        ...
        ## --------------------- SHAPE FUNCTIONS ---------------------- ##
        ##ggH
        self.signalCB_ggH = ROOT.RooCBShape("signalCB_ggH","signalCB_ggH",self.CMS_zz4l_mass, self.getVariable(self.CMS_zz4l_mean_sig_NoConv,self.rfv_mean_CB, self.bUseCBnoConvolution), self.getVariable(self.CMS_zz4l_massErr,self.rfv_sigma_CB,self.bIncludingError),self.rfv_alpha_CB,self.rfv_n_CB)
        #Low mass pdf
        #self.sig_ggH = ROOT.RooaDoubleCBxBW("sig_ggH", "sig_ggH",self.CMS_zz4l_mass,self.getVariable(self.CMS_zz4l_mean_sig_NoConv,self.rfv_mean_CB, self.bUseCBnoConvolution), self.getVariable(self.CMS_zz4l_massErr,self.rfv_sigma_CB,self.bIncludingError), self.rfv_alpha_CB, self.rfv_alpha2_CB,  self.MH, self.HiggsDecayWidth, 5, 4, 2.3, 2.3, True)
        
        self.signalBW_ggH = ROOT.RooBreitWigner("signalBW_ggH", "signalBW_ggH",self.CMS_zz4l_mass,self.MH,self.HiggsDecayWidth)
        self.sig_ggH =  ROOT.RooFFTConvPdf("sig_ggH","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_ggH,self.signalCB_ggH,2)
        #High mass pdf
        #self.signalBW_ggH_HM = ROOT.RooRelBWHighMass("signalBW_ggH", "signalBW_ggH",self.CMS_zz4l_mass,self.MH,self.rfv_gamma_BW)
        #self.sig_ggH_HM =  ROOT.RooFFTConvPdf("sig_ggH","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_ggH_HM,self.signalCB_ggH, 2)
  
        ##VBF
        self.signalCB_VBF = ROOT.RooCBShape("signalCB_VBF","signalCB_VBF",self.CMS_zz4l_mass,self.getVariable(self.CMS_zz4l_mean_sig_NoConv,self.rfv_mean_CB,self.bUseCBnoConvolution),self.getVariable(self.CMS_zz4l_massErr,self.rfv_sigma_CB, self.bIncludingError),self.rfv_alpha_CB,self.rfv_n_CB)
        #Low mass pdf
        self.signalBW_VBF = ROOT.RooBreitWigner("signalBW_VBF", "signalBW_VBF",self.CMS_zz4l_mass,self.MH,self.HiggsDecayWidth)
        self.sig_VBF = ROOT.RooFFTConvPdf("sig_VBF","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_VBF,self.signalCB_VBF, 2)
        #High mass pdf
        #self.signalBW_VBF_HM = ROOT.RooRelBWHighMass("signalBW_VBF", "signalBW_VBF",self.CMS_zz4l_mass,self.MH,self.rfv_gamma_BW)
        #self.sig_VBF_HM = ROOT.RooFFTConvPdf("sig_VBF","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_VBF_HM,self.signalCB_VBF, 2)
                       
        ##WH
        self.signalCB_WH = ROOT.RooCBShape("signalCB_WH","signalCB_WH",self.CMS_zz4l_mass,self.getVariable(self.CMS_zz4l_mean_sig_NoConv,self.rfv_mean_CB,self.bUseCBnoConvolution),self.getVariable(self.CMS_zz4l_massErr,self.rfv_sigma_CB, self.bIncludingError),self.rfv_alpha_CB,self.rfv_n_CB)
        #Low mass pdf
        self.signalBW_WH = ROOT.RooBreitWigner("signalBW_WH", "signalBW_WH",self.CMS_zz4l_mass,self.MH,self.HiggsDecayWidth)
        self.sig_WH = ROOT.RooFFTConvPdf("sig_WH","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_WH,self.signalCB_WH, 2)
        #High mass pdf
        #self.signalBW_WH_HM = ROOT.RooRelBWHighMass("signalBW_WH", "signalBW_WH",self.CMS_zz4l_mass,self.MH,self.rfv_gamma_BW)
        #self.sig_WH_HM = ROOT.RooFFTConvPdf("sig_WH","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_WH_HM,self.signalCB_WH, 2)

        ##ZH
        self.signalCB_ZH = ROOT.RooCBShape("signalCB_ZH","signalCB_ZH",self.CMS_zz4l_mass,self.getVariable(self.CMS_zz4l_mean_sig_NoConv,self.rfv_mean_CB,self.bUseCBnoConvolution),self.getVariable(self.CMS_zz4l_massErr,self.rfv_sigma_CB, self.bIncludingError),self.rfv_alpha_CB,self.rfv_n_CB)
        #Low mass pdf
        self.signalBW_ZH = ROOT.RooBreitWigner("signalBW_ZH", "signalBW_ZH",self.CMS_zz4l_mass,self.MH,self.HiggsDecayWidth)
        self.sig_ZH = ROOT.RooFFTConvPdf("sig_ZH","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_ZH,self.signalCB_ZH, 2)
        #High mass pdf
        #self.signalBW_ZH_HM = ROOT.RooRelBWHighMass("signalBW_ZH", "signalBW_ZH",self.CMS_zz4l_mass,self.MH,self.rfv_gamma_BW)
        #self.sig_ZH_HM = ROOT.RooFFTConvPdf("sig_ZH","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_ZH_HM,self.signalCB_ZH, 2)

        ##ttH
        self.signalCB_ttH = ROOT.RooCBShape("signalCB_ttH","signalCB_ttH",self.CMS_zz4l_mass,self.getVariable(self.CMS_zz4l_mean_sig_NoConv,self.rfv_mean_CB,self.bUseCBnoConvolution),self.getVariable(self.CMS_zz4l_massErr,self.rfv_sigma_CB,self.bIncludingError),self.rfv_alpha_CB,self.rfv_n_CB)
        #Low mass pdf
        self.signalBW_ttH = ROOT.RooBreitWigner("signalBW_ttH", "signalBW_ttH",self.CMS_zz4l_mass,self.MH,self.HiggsDecayWidth)
        self.sig_ttH = ROOT.RooFFTConvPdf("sig_ttH","BW (X) CB",self.CMS_zz4l_mass,self.signalBW_ttH,self.signalCB_ttH, 2) 
        #High mass pdf
        #self.signalBW_ttH_HM = ROOT.RooRelBWHighMass("signalBW_ttH", "signalBW_ttH",self.CMS_zz4l_mass,self.MH,self.rfv_gamma_BW)
	...
	#	CMS_zz4l_mass.setVal(140);
        ...
        self.CMS_qqzzbkg_a5.setVal(1)
        self.CMS_qqzzbkg_a6.setVal(1)
        self.CMS_qqzzbkg_a7.setVal(0)
        self.CMS_qqzzbkg_a8.setVal(1)
        self.CMS_qqzzbkg_a9.setVal(1)
        self.CMS_qqzzbkg_a10.setVal(1)
        self.CMS_qqzzbkg_a11.setVal(1)
        self.CMS_qqzzbkg_a12.setVal(1)
        self.CMS_qqzzbkg_a13.setVal(0)
...
#         elif (self.channel == self.ID_4e or self.channel == self.ID_2e2mu):
...
#             self.bkg_zjets = ROOT.RooGenericPdf("bkg_zjetsTmp","bkg_zjetsTmp","(TMath::Landau(@0,@1,@2))*@3*(1.+ TMath::Exp(@4+@5*@0))",RooArgList(self.CMS_zz4l_mass,self.mlZjet_2p2f,self.slZjet_2p2f,self.nlZjet_2p2f,self.p0Zjet_2p2f,self.p1Zjet_2p2f))
...
#         #self.bkg_zjets = ROOT.RooProdPdf(name,"bkg_zjetsTmp",self.bkg_zjetsLandau,self.bkg_zjetsPoly)
	...
	####  ----------------------- end mekd -----------------------------------------------------------
        ...
        self.CMS_zz4l_mass.setRange("plot",50,110)
        ## ----------------------- PLOTS FOR SANITY CHECKS -------------------------- ##
        if(self.DEBUG):
            czz = ROOT.TCanvas( "czz", "czz", 750, 700 )
            czz.cd()
            zzframe_s = self.CMS_zz4l_mass.frame(45)
            super(ROOT.RooFFTConvPdf,self.sig_ggH).plotOn(zzframe_s, ROOT.RooFit.LineStyle(1), ROOT.RooFit.LineColor(1),ROOT.RooFit.Normalization(self.inputs['signal_rate'], ROOT.RooAbsReal.NumEvent ))

            super(ROOT.RooqqZZPdf_v2,self.bkg_qqzz).plotOn(zzframe_s, ROOT.RooFit.LineStyle(1), ROOT.RooFit.LineColor(4),ROOT.RooFit.Normalization(self.inputs['qqZZ_rate'], ROOT.RooAbsReal.NumEvent ))

            ##super(ROOT.RooggZZPdf_v2,self.bkg_ggzz).plotOn(zzframe_s, ROOT.RooFit.LineStyle(1), ROOT.RooFit.LineColor(6) )
            if self.channel == self.ID_4mu: super(ROOT.RooLandau,self.bkg_zjets).plotOn(zzframe_s, ROOT.RooFit.LineStyle(2), ROOT.RooFit.LineColor(6),ROOT.RooFit.Normalization(self.inputs['zjets_rate']*(self.bkg_zjets.createIntegral( ROOT.RooArgSet(self.CMS_zz4l_mass), ROOT.RooFit.Range("plot") ).getVal()/self.bkg_zjets.createIntegral( ROOT.RooArgSet(self.CMS_zz4l_mass), ROOT.RooFit.Range("ZXFullrange") ).getVal()), ROOT.RooAbsReal.NumEvent ))
            else: super(ROOT.RooAddPdf,self.bkg_zjets).plotOn(zzframe_s, ROOT.RooFit.LineStyle(2), ROOT.RooFit.LineColor(6),ROOT.RooFit.Normalization(self.inputs['zjets_rate']*(self.bkg_zjets.createIntegral( ROOT.RooArgSet(self.CMS_zz4l_mass), ROOT.RooFit.Range("plot") ).getVal()/self.bkg_zjets.createIntegral( ROOT.RooArgSet(self.CMS_zz4l_mass), ROOT.RooFit.Range("ZXFullrange") ).getVal()), ROOT.RooAbsReal.NumEvent))
        ...
        self.CMS_zz4l_mass.setRange("fullrange",50,1400)
        self.CMS_zz4l_mass.setRange("ZXFullrange",50,200)
        self.CMS_zz4l_mass.setRange("qqZZfullrange",100,110)
...
#         print "Sum of sigRate_XYZ_Shape=",sigRate_ggH_Shape+sigRate_VBF_Shape+sigRate_WH_Shape+sigRate_ZH_Shape+sigRate_ttH_Shape
        ...
        #signal_rate = self.inputs['signal_rate']
        #self.rfvSigRate_ggH = ROOT.RooRealVar("ggH_norm","ggH_norm",signal_rate) 
        ...
        ## Get Normalizations
        normalizationBackground_qqzz = self.bkg_qqzz.createIntegral( ROOT.RooArgSet(self.CMS_zz4l_mass), ROOT.RooFit.Range("qqZZfullrange") ).getVal()
        ...
        bkgRate_qqzz_Shape = bkgRate_qqzz #sclFactorBkg_qqzz * self.bkg_qqzz.createIntegral( ROOT.RooArgSet(self.CMS_zz4l_mass), ROOT.RooFit.Range("shape") ).getVal()
        ...
        sigRate_ggH_Shape = self.inputs['signal_rate']
        sigRate_VBF_Shape = 0
        sigRate_WH_Shape = 0
        sigRate_ZH_Shape = 0
        sigRate_ttH_Shape = 0
        ...
        self.rates = {}
        self.rates['ggH'] = self.inputs['signal_rate']
    ...
    def writeWorkspace(self):
        ...
        if (self.endsInP5(self.mH)):
            self.name_ShapeWS = "{0}/HCG/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV.input.root".format(self.outputDir,self.mH,self.appendName,self.sqrts)
        else:
            self.name_ShapeWS = "{0}/HCG/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV.input.root".format(self.outputDir,self.mH,self.appendName,self.sqrts)
            
        if (self.endsInP5(self.mH)):
            self.name_ShapeWSXSBR = "{0}/HCG_XSxBR/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV.input.root".format(self.outputDir,self.mH,self.appendName,self.sqrts)
        else:
            self.name_ShapeWSXSBR = "{0}/HCG_XSxBR/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV.input.root".format(self.outputDir,self.mH,self.appendName,self.sqrts)
        ...
        self.w.importClassCode(RooBreitWigner.Class(),True)
        self.w.importClassCode(RooCBShape.Class(),True)
    ...
    def prepareDatacard(self):
        ...
        if (self.endsInP5(self.mH)):
            name_Shape = "{0}/HCG/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV.txt".format(self.outputDir,self.mH,self.appendName,self.sqrts)
        else:
            name_Shape = "{0}/HCG/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV.txt".format(self.outputDir,self.mH,self.appendName,self.sqrts)
        ...
        if(self.isAltSig):
            if (self.endsInP5(self.mH)):
                name_Shape = "{0}/HCG/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV{4}.txt".format(self.outputDir,self.mH,self.appendName,self.sqrts,self.appendHypType)
            else:
                name_Shape = "{0}/HCG/{1:.4f}/hzz4l_{2}S_{3:.0f}TeV{4}.txt".format(self.outputDir,self.mH,self.appendName,self.sqrts,self.appendHypType)
            fo = open( name_Shape, "wb")
        ...
        ## forXSxBR
        if (self.endsInP5(self.mH)):
            name_Shape = "{0}/HCG_XSxBR/{2:.4f}/hzz4l_{1}S_{3:.0f}TeV.txt".format(self.outputDir,self.appendName,self.mH,self.sqrts)	
        else:
            name_Shape = "{0}/HCG_XSxBR/{2:.4f}/hzz4l_{1}S_{3:.0f}TeV.txt".format(self.outputDir,self.appendName,self.mH,self.sqrts)
	...
	self.rfv_EBE_sig_ld_sigma = ROOT.RooFormulaVar(name, "("+self.inputs['relerr_ggH_ld_sigma']+")", ROOT.RooArgList(self.MH))
	name = "CMS_zz4l_massErrS_ld_mean_{0:.0f}".format(self.channel)
	self.rfv_EBE_sig_ld_mean = ROOT.RooFormulaVar(name, "("+self.inputs['relerr_ggH_ld_mean']+")", ROOT.RooArgList(self.MH)) 
	...
	self.rfv_EBE_zz_ld_sigma = ROOT.RooFormulaVar(name, "("+self.inputs['relerr_qqzz_ld_sigma']+")", ROOT.RooArgList(self.CMS_zz4l_mass))
	name = "CMS_zz4l_massErrZZ_ld_mean_{0:.0f}".format(self.channel)
	self.rfv_EBE_zz_ld_mean = ROOT.RooFormulaVar(name, "("+self.inputs['relerr_qqzz_ld_mean']+")", ROOT.RooArgList(self.CMS_zz4l_mass)) 
	...
	name = "CMS_zz4l_massErrZZ_ld_frac_{0:.0f}".format(self.channel)
        self.MHTmp = ROOT.RooRealVar("MHTmp","MHTmp",110)
        self.MHTmp.setConstant(True)
	...
	self.rfv_EBE_zx_ld_sigma = ROOT.RooFormulaVar(name, "("+self.inputs['relerr_zx_ld_sigma']+")", ROOT.RooArgList(self.CMS_zz4l_mass)); 
	name = "CMS_zz4l_massErrZX_ld_mean_{0:.0f}".format(self.channel);
	self.rfv_EBE_zx_ld_mean = ROOT.RooFormulaVar(name, "("+self.inputs['relerr_zx_ld_mean']+")", ROOT.RooArgList(self.CMS_zz4l_mass)); 
	...
	self.sig_ggHErr = ROOT.RooProdPdf("sig_ggHErr","BW (X) CB * pdfErr", ROOT.RooArgSet(self.sig_ggH), ROOT.RooFit.Conditional(ROOT.RooArgSet(self.pdfErrS), ROOT.RooArgSet(self.RelErr)));
	self.sig_VBFErr = ROOT.RooProdPdf("sig_VBFErr","BW (X) CB * pdfErr", ROOT.RooArgSet(self.sig_VBF), ROOT.RooFit.Conditional(ROOT.RooArgSet(self.pdfErrS), ROOT.RooArgSet(self.RelErr)));
	self.sig_WHErr = ROOT.RooProdPdf("sig_WHErr","BW (X) CB * pdfErr", ROOT.RooArgSet(self.sig_WH), ROOT.RooFit.Conditional(ROOT.RooArgSet(self.pdfErrS), ROOT.RooArgSet(self.RelErr)));
	self.sig_ZHErr = ROOT.RooProdPdf("sig_ZHErr","BW (X) CB * pdfErr", ROOT.RooArgSet(self.sig_ZH), ROOT.RooFit.Conditional(ROOT.RooArgSet(self.pdfErrS), ROOT.RooArgSet(self.RelErr)));
	self.sig_ttHErr = ROOT.RooProdPdf("sig_ttHErr","BW (X) CB * pdfErr", ROOT.RooArgSet(self.sig_ttH), ROOT.RooFit.Conditional(ROOT.RooArgSet(self.pdfErrS), ROOT.RooArgSet(self.RelErr)));
